...
def update_script() -> bool:
    ...
    try:
        ...
        version_match = re.search(r'__version__\s*=\s*[\'"]([^\'"]+)[\'"]', new_content)
        if not version_match:
            logger.error("Could not determine version from GitHub script")
            print("[!] Update failed: Could not parse version from remote script.")
            return False
        new_version = version_match.group(1)

        def version_tuple(ver: str):
            return tuple(int(x) if x.isdigit() else 0 for x in re.findall(r'\d+', ver))

        if version_tuple(new_version) <= version_tuple(__version__):
            logger.info("Already running latest version (%s)", __version__)
            print(f"Recon-Cell  is up to date (v{__version__})")
            return True
...
def run_command(cmd: Union[str, List[str]], tool_name: str) -> Optional[str]:
    try:
        if isinstance(cmd, str):
            cmd_list = re.split(r'\s+', cmd.strip())
        else:
            cmd_list = cmd
...
async def async_main() -> None:
    ...
    # --- Handle update request and exit ---
    if args.update:
        if update_script():
            sys.exit(0)
        else:
